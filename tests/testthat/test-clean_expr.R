...
test_that("parentesis are properly cleaned", {
  ...
  expect_equal(clean_expr("(1 + 4) + ((4 - 4) + 9)"), "1 + 4 + 4 - 4 + 9")
  expect_equal(clean_expr("+ (+ 3 * 2) / (-4 - 9)"), "3 * 2 / (-4 - 9)")
  expect_equal(clean_expr("1 / (5 / (-1))"), "1 / (5 / (-1))")
  expect_equal(clean_expr("4 + (- 4)"), "4 + (- 4)")
  expect_equal(clean_expr("- (5 * 3 / 2)"), "- 5 * 3 / 2")
